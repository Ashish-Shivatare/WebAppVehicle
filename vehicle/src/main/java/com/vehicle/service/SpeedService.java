...
@WebServlet("/speed")
public class SpeedService extends HttpServlet {
	private static final long serialVersionUID = 1L;

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		VehicleDao vehicleDao = new VehicleDao();
		List<Vehicle> maxSpeedList = new ArrayList<Vehicle>();
		maxSpeedList = vehicleDao.displayByName();
		resp.setContentType("text/html");
		PrintWriter pw=resp.getWriter();
		pw.println("<h1>Vehicle details displayed by name and max speed:</h1>");
		...
		pw.print("<tbody>");
		for (Vehicle vlist : maxSpeedList) {
			pw.print("<tr>");
			pw.print("<td>"+vlist.getVehicleId()+"</td>");
			pw.print("<td>"+vlist.getVehicleType()+"</td>");
			pw.print("<td style='color:red'>"+vlist.getVehicleName()+"</td>");
			pw.print("<td>"+vlist.getVehicleModel()+"</td>");
			pw.print("<td>"+vlist.getVehicleDate()+"</td>");
			...
			pw.print("<td style='color:red'>"+vlist.getVehicleMaxSpeed()+"</td>");
